...
# --first-parent ensures we don't follow tags not published in master through
# an unlikely intermediary merge commit.
last_tagged_commit=$(git describe --tags --abbrev=0 --first-parent)

# Check if any file that has not be listed in IGNORE_DIFF_ON has changed since
# the last tag was published.
